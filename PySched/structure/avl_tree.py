...
class AVLTree():
    ...
    def add_as_child (self, parent_node, child_node, union=False):
        # print 'add %s under %s'%(child_node.key, parent_node.key)
        
        node_to_rebalance = None
        if union:
            pa,pb = parent_node.key
            ca,cb = child_node.key

            if pa == cb:
                
                # check if bridge
                n = parent_node.leftChild
                while n is not None:
                    la,lb = n.key
                    if lb == ca:
                        ca = la
                ...
                parent_node.key = ca,pb
                return 0
            elif ca == pb:
                
                # check if bridge
                n = parent_node.rightChild
                while n is not None:
                    ra,rb = n.key
                    if ra == cb:
                        cb = rb
        ...
        if child_node.key < parent_node.key:
            if not parent_node.leftChild:
                ...
                res = self.add_as_child(parent_node.leftChild, child_node, union)
        else:
            if not parent_node.rightChild:
                ...
                res = self.add_as_child(parent_node.rightChild, child_node, union)
    ...
    def myinsert (self, key):
        ...
        if not self.rootNode:
            ...
            self.elements_count += self.add_as_child (self.rootNode, new_node, union=True)
    ...
    # def find_lb_in_subtree (self, node, a):
    #     if node is None:
    #         return None  # key not found
    #     if a == node.key[0]:
    #         return node
    #     elif a < node.key[0]:
    #         return self.find_lb_in_subtree(node.leftChild, a)
    #     return None
