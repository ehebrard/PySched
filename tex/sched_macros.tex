...
\def\ji{\ensuremath{l}}
\def\mi{\ensuremath{j}}
\def\alltasks{\ensuremath{\cal T}}
\def\schedule{\ensuremath{\cal S}}


\def\atask{\ensuremath{a}}
...
\newcommand{\usagevar}[2]{\ensuremath{u^{#2}_{#1}}}
...
stresAB/.style={color=carnelian, fill=white!#2!blue(munsell)},
stresAC/.style={color=chocolate(web), fill=white!#2!blue(munsell)},
stresAD/.style={color=darkgreen, fill=white!#2!blue(munsell)},
...
stresBD/.style={color=darkgreen, fill=white!#2!cadmiumred},
...
\newcommand\NormalTask[6]{ %1:start, 2:duration, 3:demand, 4:row, 5:color, 6:label 
		\LeftVariableTask{#1}{#2}{#2}{#3}{#4}{#5}{#6}
}
...
\newcommand\PhantomUnlabeledGroundTask[6]{ %1:start, 2:duration, 3:demand, 4:row, 5:color, 6:label 
    \draw[opacity=.3, thick, \getStyle{#5}, rounded corners] (#1,-#4-#3) rectangle (#1+#2,-#4);
    \node[minimum width=#2*.48cm] (t) at (#1+#2/2,-#4-#3/2) {{\scriptsize #6}};
}
...
\newcommand\PhantomExtensibleTask[7]{ %1:start, 2:min duration, 3:max duration, 4:demand, 5:row, 6:color, 7:label 
		...
		% \draw[thick, opacity=.2, \getStyle{#6}] (#1+#3/2-#2/2-\tasksep,-#5-#4+\tasksep*2) rectangle (#1+#3/2+#2/2-\tasksep,-\tasksep*2-#5);
		%     \node[] (t#7) at (#1+#3/2,-#5-#4/2) {{\scriptsize #7}};
    \draw[thick, opacity=.2, \getExtStyle{#6}, rounded corners] (#1,-#5-#4+\tasksep) rectangle (#1+#3,-\tasksep-#5);
...
\newcommand\PrintScaledGrid[3] {
	\draw[densely dotted, color=black!90, xstep={#3}] (0, 0) grid (#1, -#2);
}
	...
	\printrow{#2}
	  ...
	  \foreach \imach/\tdur/\ijob in #3 {
			\ifnum\value{\getResource{\imach}}<\value{\getJob{\ijob}}
					\NormalTask{\value{\getJob{\ijob}}}{\tdur}{1}{\value{\getRow{\ijob}}}{\imach}{}
					\addtocounter{\getJob{\ijob}}{\tdur}
					\setcounter{\getResource{\imach}}{\value{\getJob{\ijob}}}
	 			\else
					\NormalTask{\value{\getResource{\imach}}}{\tdur}{1}{\value{\getRow{\ijob}}}{\imach}{}
...
\newcommand\Profile[3] {
	...
	\foreach \X/\Y in #1 {
		...
		\draw[ultra thick, color=bostonuniversityred] (\arabic{x}, \arabic{py}-#3) -- (\arabic{x}, \arabic{y}-#3) -- (\X, \arabic{y}-#3) -- (\X,\Y-#3);
