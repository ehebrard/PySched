...
\begin{frame}
	...
	\begin{myblock}{A model of {\sc CumulativeResource}}
		\begin{itemize}
			\item \memph{Processing time}: require the resources for at least $\dur{i}$ time \hfill $\stof{i} + \dur{i} \leq \ctof{i}~\forall i$
			\item \memph{Non preemption}: cannot be interrupted \hfill $\stof{i} + \dur{i} \geq \ctof{i}~\forall i$
