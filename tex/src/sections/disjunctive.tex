...
\begin{itemize}
	...
	\end{itemize}
	\uncover<2->{
	\item To keep the property that start times do not need to be instanciated:
	
	\begin{itemize}
	\item For every \memph{minimal} subset \memph{$S$} of tasks such that \memph{$\sum_{i \in S}\demand{i} > \capacity$} to post:
	% \begin{itemize}
		% \item
		\memph{$$\bigvee_{i \neq j \in S} i \prec j$$}
	% \end{itemize}
