...
\begin{columns}
	...
	\begin{itemize}
		\item An interval \memph{$[a,b)$}, a task \memph{$i$}
		\begin{itemize}
			\item \memph{overload} iff \memph{$\demand{i}+h > \capacity$}
			\item \memph{forbidden} iff overload \& \memph{$\lst{i} > a$} (not counted)
			\item \memph{relevant} iff \memph{$\ect{i}$}
		\end{itemize}
		%
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\uncover<4->{
			\item {\scriptsize find \memph{$i$}'s rightmost relevant \& forbidden interval}
			\begin{itemize}
